<template>
  <div class="flex items-center justify-between px-3 md:px-5 border-b"
    :style="{
      height: layoutMode === 'mobile' ? 'auto' : '48px',
    ...
    <template v-if="layoutMode === 'desktop'">
      <div class="flex items-center gap-3 min-w-0">
          ...
          <svg class="w-7 h-7" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <linearGradient id="logo-bg" x1="0" y1="0" x2="1" y2="1">
                <stop offset="0%" stop-color="#4f46e5"/><stop offset="50%" stop-color="#6d28d9"/><stop offset="100%" stop-color="#7c3aed"/>
              </linearGradient>
        ...
        <span class="text-[10px] font-medium px-2 py-0.5 rounded-full shrink-0"
          :style="{ background: 'var(--accent-subtle)', color: 'var(--accent)' }">Mermaid</span>
        <div class="flex items-center rounded-lg p-0.5" :style="{ background: 'var(--bg-tertiary)' }">
          <button class="px-2.5 py-1 text-xs rounded-md transition-all duration-200 cursor-pointer font-medium"
            :style="editorMode === 'visual' ? { background: 'var(--bg-elevated)', color: 'var(--text-primary)', boxShadow: 'var(--shadow-sm)' } : { color: 'var(--text-tertiary)' }"
            @click="$emit('editorModeChange', 'visual')">可视化</button>
          <button class="px-2.5 py-1 text-xs rounded-md transition-all duration-200 cursor-pointer font-medium"
            :style="editorMode === 'code' ? { background: 'var(--bg-elevated)', color: 'var(--text-primary)', boxShadow: 'var(--shadow-sm)' } : { color: 'var(--text-tertiary)' }"
            @click="$emit('editorModeChange', 'code')">代码</button>
        </div>
      </div>
      <div class="flex items-center gap-1">
        <button class="premium-btn px-3 py-1.5 text-sm rounded-lg flex items-center gap-1.5 cursor-pointer font-medium"
          :style="{ color: 'var(--text-secondary)' }" @click="$emit('openTemplate')"
          ...
          模板</button>
        <button class="premium-btn px-3 py-1.5 text-sm rounded-lg flex items-center gap-1.5 cursor-pointer font-medium"
          :style="{ color: 'var(--text-secondary)' }" @click="handleImport"
          @mouseenter="($event.target as HTMLElement).style.background = 'var(--bg-tertiary)'"
          @mouseleave="($event.target as HTMLElement).style.background = 'transparent'">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
          导入</button>
        <button class="premium-btn px-3 py-1.5 text-sm rounded-lg flex items-center gap-1.5 cursor-pointer font-medium"
          :style="{ color: 'var(--text-secondary)' }" @click="handleSave"
          @mouseenter="($event.target as HTMLElement).style.background = 'var(--bg-tertiary)'"
          @mouseleave="($event.target as HTMLElement).style.background = 'transparent'">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M5 3h11l5 5v12a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M7 3v6h8V3"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M7 21v-6h10v6"/></svg>
          保存</button>
        <div class="w-px h-5 mx-1" :style="{ background: 'var(--border-primary)' }" />
        <ExportMenu :chart-element="chartElement" :theme="theme" :code="code" />
        <div class="w-px h-5 mx-1" :style="{ background: 'var(--border-primary)' }" />
        <button class="premium-btn px-3 py-1.5 text-sm rounded-lg flex items-center gap-1.5 cursor-pointer font-medium"
          :style="{ color: 'var(--text-secondary)' }" @click="$emit('themeChange', theme === 'dark' ? 'light' : 'dark')"
          @mouseenter="($event.target as HTMLElement).style.background = 'var(--bg-tertiary)'"
          @mouseleave="($event.target as HTMLElement).style.background = 'transparent'">
          <svg v-if="theme === 'dark'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
      ...
      <div class="flex items-center gap-2 min-w-0 w-full">
        <h1 class="text-sm font-semibold flex items-center gap-1.5 shrink-0" style="color: var(--text-primary);">
          <svg class="w-6 h-6" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
            <defs><linearGradient id="logo-bg-t" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="#4f46e5"/><stop offset="50%" stop-color="#6d28d9"/><stop offset="100%" stop-color="#7c3aed"/></linearGradient></defs>
            <rect width="48" height="48" rx="12" fill="url(#logo-bg-t)"/>
        ...
        <span class="text-[10px] font-medium px-1.5 py-0.5 rounded-full shrink-0"
          :style="{ background: 'var(--accent-subtle)', color: 'var(--accent)' }">Mermaid</span>
        <!-- 编辑模式切换 -->
        <div class="flex items-center rounded-lg p-0.5 shrink-0" :style="{ background: 'var(--bg-tertiary)' }">
          <button class="px-2.5 py-1 text-xs rounded-md transition-all duration-200 cursor-pointer font-medium"
            :style="editorMode === 'visual' ? { background: 'var(--bg-elevated)', color: 'var(--text-primary)', boxShadow: 'var(--shadow-sm)' } : { color: 'var(--text-tertiary)' }"
            @click="$emit('editorModeChange', 'visual')">可视化</button>
          <button class="px-2.5 py-1 text-xs rounded-md transition-all duration-200 cursor-pointer font-medium"
        ...
        <!-- 操作按钮 -->
        <button class="premium-btn px-2.5 py-1.5 text-xs rounded-lg flex items-center gap-1 cursor-pointer font-medium"
          :style="{ color: 'var(--text-secondary)' }" @click="$emit('openTemplate')"
          @mouseenter="($event.target as HTMLElement).style.background = 'var(--bg-tertiary)'"
          @mouseleave="($event.target as HTMLElement).style.background = 'transparent'">
        ...
        <button class="premium-btn px-2.5 py-1.5 text-xs rounded-lg flex items-center gap-1 cursor-pointer font-medium"
          ...
          @mouseenter="($event.target as HTMLElement).style.background = 'var(--bg-tertiary)'"
          @mouseleave="($event.target as HTMLElement).style.background = 'transparent'">
          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
          导入</button>
        <button class="premium-btn px-2.5 py-1.5 text-xs rounded-lg flex items-center gap-1 cursor-pointer font-medium"
          :style="{ color: 'var(--text-secondary)' }" @click="handleSave"
          @mouseenter="($event.target as HTMLElement).style.background = 'var(--bg-tertiary)'"
          ...
          保存</button>
        <ExportMenu :chart-element="chartElement" :theme="theme" :code="code" />
        <button class="premium-btn p-2 rounded-lg cursor-pointer" :style="{ color: 'var(--text-secondary)' }"
    ...
    <template v-else>
      <div class="w-full py-1.5">
        <div class="flex items-center justify-between">
          <h1 class="text-sm font-semibold flex items-center gap-1.5 shrink-0" style="color: var(--text-primary);">
            <svg class="w-6 h-6" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
              <defs><linearGradient id="logo-bg-m" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="#4f46e5"/><stop offset="50%" stop-color="#6d28d9"/><stop offset="100%" stop-color="#7c3aed"/></linearGradient></defs>
          ...
          <div class="flex items-center gap-0.5">
            ...
            <button class="premium-btn p-2 rounded-lg cursor-pointer" :style="{ color: 'var(--text-secondary)' }" @click="handleImport" title="导入">
              <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
            </button>
            <ExportMenu :chart-element="chartElement" :theme="theme" :code="code" />
            <button class="premium-btn p-2 rounded-lg cursor-pointer" :style="{ color: 'var(--text-secondary)' }" @click="$emit('themeChange', theme === 'dark' ? 'light' : 'dark')" title="切换主题">
              ...
              <svg v-else class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/></svg>
            </button>
          </div>
        </div>
        <div class="flex items-center gap-2 mt-1.5">
          <div class="flex items-center rounded-lg p-0.5 flex-1" :style="{ background: 'var(--bg-tertiary)' }">
            <button class="flex-1 px-3 py-1.5 text-xs rounded-md transition-all duration-200 cursor-pointer font-medium text-center"
              :style="mobileTab === 'editor' ? { background: 'var(--bg-elevated)', color: 'var(--text-primary)', boxShadow: 'var(--shadow-sm)' } : { color: 'var(--text-tertiary)' }"
            ...
            <button class="flex-1 px-3 py-1.5 text-xs rounded-md transition-all duration-200 cursor-pointer font-medium text-center"
              :style="mobileTab === 'preview' ? { background: 'var(--bg-elevated)', color: 'var(--text-primary)', boxShadow: 'var(--shadow-sm)' } : { color: 'var(--text-tertiary)' }"
          ...
          <div v-if="mobileTab === 'editor'" class="flex items-center rounded-lg p-0.5 shrink-0" :style="{ background: 'var(--bg-tertiary)' }">
            <button class="px-2 py-1.5 text-[11px] rounded-md transition-all duration-200 cursor-pointer font-medium"
              :style="editorMode === 'visual' ? { background: 'var(--bg-elevated)', color: 'var(--text-primary)', boxShadow: 'var(--shadow-sm)' } : { color: 'var(--text-tertiary)' }"
              @click="$emit('editorModeChange', 'visual')">可视化</button>
            <button class="px-2 py-1.5 text-[11px] rounded-md transition-all duration-200 cursor-pointer font-medium"
              :style="editorMode === 'code' ? { background: 'var(--bg-elevated)', color: 'var(--text-primary)', boxShadow: 'var(--shadow-sm)' } : { color: 'var(--text-tertiary)' }"
